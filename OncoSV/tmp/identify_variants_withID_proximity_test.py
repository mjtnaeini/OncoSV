...
def identify_variants(tumour, normal, chromosomes, default_window_size=20, large_window_size=50):
    ...
    for index, tumour_row in tumour.iterrows():
        ...
        svtype = tumour_row['SVTYPE']  # SVTYPE of the tumour variant
        
        # Ensure SVLEN is absolute for deletions
        tumour_svlen = abs(tumour_row['SVLEN']) if svtype == 'DEL' else tumour_row['SVLEN']

        # Use different window sizes based on SVTYPE
        window_size = large_window_size if svtype in ['INS', 'DUP', 'DEL'] else default_window_size
        ...
        if svtype in ['DUP', 'DEL'] and tumour_svlen > 1000:
            # Match only by CHROM and POS for these SV types
            match_normal = normal[
            ...
            # Match using full CHROM1, CHROM2, POS1, POS2 logic for other SV types
            match_normal = normal[
                (normal['CHROM'] == chrom1) & (normal['POS'] >= window_start1) & (normal['POS'] <= window_end1) &
        ...
        if not match.empty:
            ...
            # Check SVLEN condition for SVTYPE = INS
            ...
            # Variant found only in tumour (somatic)
            tumour.at[index, 'variant_type'] = 'somatic'
            tumour.at[index, 'variant_ID'] = f'somatic.{svtype}.{variant_counter}'
            variant_counter += 1
